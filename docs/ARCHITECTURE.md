...
- **`ui/main/FunctionGemmaVoiceProposalHandler.kt`**: Converts validated FunctionGemma proposal results into existing voice UI states. It accepts read-latest proposals and live active-notification reply proposals, rejects expired notification keys, and fails safely for tools not yet wired to the voice UI.
