...
- **`ai/tools/WhatsAppTools.kt`**: Active Gallery-style LiteRT-LM `ToolSet` adapter using `@Tool` / `@ToolParam` annotations. It exposes an expanded side-effect-free callback surface and delegates to a JVM-testable handler.
- **`ai/tools/WhatsAppToolRegistry.kt`**: Typed Kotlin mirror of the documented 16-tool app-level action contract. Each tool is assigned a safety level: read-only, local write, confirmation-required, external app open, send message, or delete data.
- **`notifications/NotificationSourceModels.kt`**: Generic source catalog and common query/event projection types for WhatsApp, SMS, Gmail, Phone, Calendar, and Other. Only WhatsApp is enabled for V1 production capture/reply.
- **`ai/tools/WhatsAppToolActionHandler.kt`**: Dependency-free action boundary for model tool callbacks. It validates and normalizes model parameters, captures typed `WhatsAppToolAction` values, and returns model-safe result maps.
...
- **`ai/tools/ToolSafetyRouter.kt`**: Converts validated proposals into explicit execution decisions. This keeps read-only local data access, local writes, external app opens, strict send confirmation, strict delete confirmation, and rejection separate from parsing.
...
- **`ai/tools/PhoneContextSnapshot.kt` / `ai/tools/GemmaPromptBuilder.kt`**: Builds bounded, recency-sorted prompt context from selected local WhatsApp messages plus live reply-target keys. It renders active notifications, unread chat summaries, and relevant stored messages without dumping the whole inbox.
...
- **`ai/runtime/LiteRtGemmaEngine.kt`**: Isolated LiteRT-LM engine/conversation adapter following Gallery's engine initialize, createConversation, sendMessageAsync, and close pattern. It is backend-aware and uses automatic tool callbacks for the side-effect-free native `WhatsAppTools` proposal surface.
