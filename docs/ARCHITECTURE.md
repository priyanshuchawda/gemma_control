...
- **`ai/tools/WhatsAppLocalToolExecutor.kt`**: Executes only local repository/preference operations after routing and confirmation. It can pause/resume capture, create/list/complete follow-ups, update local message priority, and delete all or conversation-scoped local WhatsApp data; Android `RemoteInput` replies remain outside this executor.
