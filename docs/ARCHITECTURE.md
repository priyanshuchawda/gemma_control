...
- **`ai/tools/WhatsAppLocalToolExecutor.kt`**: Executes only local repository/preference operations after routing and confirmation. It can pause/resume capture, search/read local messages, show the actionable inbox, create/list/complete follow-ups, update local message priority, schedule encrypted local reminders, and delete all or conversation-scoped local WhatsApp data; Android `RemoteInput` replies remain outside this executor.
