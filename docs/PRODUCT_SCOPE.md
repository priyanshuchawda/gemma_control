...
- **Limited Speech Input in V1**: English voice commands are supported for reading active WhatsApp notification messages, explicitly reading locally stored captured history, searching locally captured messages, listing pending follow-ups/actionable items, preparing a reply to the latest active WhatsApp notification, and creating local follow-up/reminder/priority proposals by message id. Sending and local writes still require manual confirmation.
