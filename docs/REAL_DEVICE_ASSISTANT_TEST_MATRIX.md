...
| RD-009 | Stored inbox read | Storage enabled with safe test messages | Use `Read my latest stored messages`. | Reads latest locally stored captured messages, not the user's outgoing reply echo. | Spoken/UI count and content-kind labels, no private text in PR. |
