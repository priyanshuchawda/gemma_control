...
- `AssistantPlanner` is the shared voice/typed command entry point. It converts text input into an explicit plan: deterministic read command, deterministic reply command, FunctionGemma proposal request with a clarification fallback, or immediate clarification for incomplete commands.
