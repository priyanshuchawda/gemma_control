...
- `WhatsAppLocalToolExecutor` executes only confirmed local-safe operations that already have Kotlin repository boundaries, currently capture pause/resume plus all-data or conversation-scoped local deletion. Active notification replies are intentionally rejected here and must use the dedicated notification reply executor.
