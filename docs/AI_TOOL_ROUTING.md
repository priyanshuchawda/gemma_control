...
- `AssistantPlanner` is the shared voice/typed command entry point. It converts text input into an explicit plan: deterministic read command, latest-reply command, named-reply command, FunctionGemma proposal request with a clarification fallback, or immediate clarification for incomplete commands.
...
- Deterministic voice reads are adaptive: 1-3 messages are read directly, larger sets are summarized by chat, and follow-up phrases such as "continue", "read more", "read messages from Mom", "summarize WhatsApp", and "only important" stay on the local TTS path.
